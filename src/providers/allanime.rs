...
impl AllAnimeProvider {
    pub fn new() -> Self {
        ...
        headers.insert(header::REFERER, header::HeaderValue::from_static(ALLANIME_REFERRER));
    ...
    pub fn decode_provider_id(encoded: &str) -> String {
        ...
        for chunk in chars.chunks(2) {
            if chunk.len() == 2 {
                ...
                if let Ok(byte) = u8::from_str_radix(&hex, 16) {
                    let ch = match byte {
                        0x79 => 'A', 0x7a => 'B', 0x7b => 'C', 0x7c => 'D',
                        0x7d => 'E', 0x7e => 'F', 0x7f => 'G', 0x70 => 'H',
                        0x71 => 'I', 0x72 => 'J', 0x73 => 'K', 0x74 => 'L',
                        0x75 => 'M', 0x76 => 'N', 0x77 => 'O', 0x68 => 'P',
                        0x69 => 'Q', 0x6a => 'R', 0x6b => 'S', 0x6c => 'T',
                        0x6d => 'U', 0x6e => 'V', 0x6f => 'W', 0x60 => 'X',
                        0x61 => 'Y', 0x62 => 'Z', 0x59 => 'a', 0x5a => 'b',
                        0x5b => 'c', 0x5c => 'd', 0x5d => 'e', 0x5e => 'f',
                        0x5f => 'g', 0x50 => 'h', 0x51 => 'i', 0x52 => 'j',
                        0x53 => 'k', 0x54 => 'l', 0x55 => 'm', 0x56 => 'n',
                        0x57 => 'o', 0x48 => 'p', 0x49 => 'q', 0x4a => 'r',
                        0x4b => 's', 0x4c => 't', 0x4d => 'u', 0x4e => 'v',
                        0x4f => 'w', 0x40 => 'x', 0x41 => 'y', 0x42 => 'z',
                        0x08 => '0', 0x09 => '1', 0x0a => '2', 0x0b => '3',
                        0x0c => '4', 0x0d => '5', 0x0e => '6', 0x0f => '7',
                        0x00 => '8', 0x01 => '9', 0x15 => '-', 0x16 => '.',
                        0x67 => '_', 0x46 => '~', 0x02 => ':', 0x17 => '/',
                        0x07 => '?', 0x1b => '#', 0x63 => '[', 0x65 => ']',
                        0x78 => '@', 0x19 => '!', 0x1c => '$', 0x1e => '&',
                        0x10 => '(', 0x11 => ')', 0x12 => '*', 0x13 => '+',
                        0x14 => ',', 0x03 => ';', 0x05 => '=', 0x1d => '%',
        ...
        result.replace("/clock", "/clock.json").replace("/clock.json.json", "/clock.json")
    }
...
impl AnimeProvider for AllAnimeProvider {
    ...
    async fn search(&self, query: &str) -> Result<Vec<Anime>> {
        ...
        if let Some(edges) = response["data"]["shows"]["edges"].as_array() {
            for edge in edges {
                ...
                let episodes = edge["availableEpisodes"]["sub"]
                    .as_u64()
                    .map(|n| n as u32);
    ...
    async fn get_episodes(&self, anime_id: &str) -> Result<Vec<Episode>> {
        ...
        if let Some(episode_list) = response["data"]["show"]["availableEpisodesDetail"]["sub"].as_array() {
    ...
    async fn get_stream_url(&self, episode_id: &str) -> Result<StreamInfo> {
        ...
        if let Some(source_urls) = response["data"]["episode"]["sourceUrls"].as_array() {
            ...
            let priority_sources = ["Ok", "Mp4", "Fm-Hls", "Yt-mp4", "S-mp4", "Luf-Mp4", "Sup", "Uni"];
            
            for priority_name in &priority_sources {
                if let Some(source) = source_urls.iter().find(|s| {
                    s["sourceName"].as_str().map_or(false, |n| n == *priority_name)
                }) {
        ...
        if stream_url.is_empty() {
            anyhow::bail!("No working stream URL found. This might be a temporary issue with AllAnime.");
        }
